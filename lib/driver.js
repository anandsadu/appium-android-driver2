...
class AndroidUiautomator2Driver extends BaseDriver {
  ...
  async startAndroidSession () {
    ...
    // start UiAutomator2
    this.uiautomator2 = new helpers.uiautomator2(this.adb, DEVICE_PORT, this.opts.websocket);
    await this.uiautomator2.start(this.opts.appPackage, this.opts.disableAndroidWatchers);
    // handling unexpected shutdown
    this.uiautomator2.onUnexpectedShutdown.catch(async (err) => {
  ...
  async setCompressedLayoutHierarchy (compress) {
    await this.uiautomator2.sendAction("compressedLayoutHierarchy", {compressLayout: compress});
  }
